...
export const SingleProductNeededSection = ({ neededProducts }) => {
  ...
  if (neededProducts.length > 0) {
    return (
      <Row style={{ paddingBottom: "10vmin" }}>
        <Col sm={12}>
          <Text type="headline2">{t("products.product.mightNeed")}</Text>
          <Swiper
          ...
          >
            {neededProducts.map((product, index) => (
              <SwiperSlide key={index}>
                <div className="other-stuff-item">
                  ...
                  <div className="other-stuff-item__content">
                    ...
                    >
                      {product.list_price} <RiyalSymbol size={12} />
                    </Text>
                  </div>
                </div>
              </SwiperSlide>
